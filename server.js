...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Roboto', sans-serif; }
        body { background-color: var(--es-bg); color: var(--es-text); display: flex; height: 100vh; overflow: hidden; }

        /* POPRAVLJEN I CENTRIRAN LOGIN EKRAN - KAO NA PRAVOM SAJTU */
        .full-login-overlay { 
            position: fixed; 
            top: 0; 
            left: 0; 
        ...
        #glavni-interfejs-aplikacije { display: none; width: 100%; height: 100%; }

        /* BOČNI MENI */
        .sidebar { width: 240px; background-color: white; border-right: 1px solid var(--es-border); display: flex; flex-direction: column; height: 100%; }
        ...
        .logo-area span { font-size: 11px; display: block; color: #a5c2dc; font-weight: normal; margin-top: 2px; }
        ...
        /* HEADER */
        .header { background: linear-gradient(90deg, #0e2a44 0%, #1c5484 60%, #2c7cb4 100%); color: white; padding: 10px 25px; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
        .school-selectors { display: flex; gap: 20px; }
        .sel-block { display: flex; flex-direction: column; }
        .sel-block span { font-size: 10px; color: #a5c2dc; text-transform: uppercase; font-weight: bold; margin-bottom: 2px; }
        .es-select { background: rgba(255,255,255,0.15); color: white; border: 1px solid rgba(255,255,255,0.25); padding: 5px 10px; border-radius: 4px; font-weight: bold; outline: none; cursor: pointer; }
        ...
        .user-role { font-size: 10px; color: #a5c2dc; font-weight: bold; }
        .user-name { font-size: 14px; font-weight: bold; }
        .user-avatar { width: 40px; height: 40px; background: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: var(--es-blue); font-size: 18px; border: 2px solid rgba(255,255,255,0.8); }
        ...
        /* IMENIK GRID SA SMART BEDŽEVIMA */
        ...
        .modal-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.4); display: none; align-items: center; justify-content: center; z-index: 10000; }
        .modal-box { background: white; border-radius: 8px; width: 500px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.2); }
        .modal-header { background: var(--es-dark-blue); color: white; padding: 15px 20px; font-weight: bold; display: flex; justify-content: space-between; align-items: center; }
...
<body>
    ...
    <div id="stranica-login-sistema" class="full-login-overlay">
        <div class="login-box">
            ...
            <p>Пријава на систем за наставнике и админе</p>
    ...
    <div id="glavni-interfejs-aplikacije">
            ...
            <div class="header">
                ...
                <div class="user-panel">
                    <div class="user-text">
                        ...
                        <p class="user-name" id="header-nastavnik-ime">Стефан Михајловић</p>
                    </div>
            ...
            <div class="content-area">
                    ...
                    <div class="profile-container">
                        ...
                        <div class="profile-right">
                            ...
                            <div id="tab-dosije-ocene" class="tab-content active">
                                ...
                                <br><br>
                                <h4>Историја и измена појединачних оцена и активности:</h4>
                ...
                <div id="stranica-tok-casa" class="page-section">
                    <div class="form-card" style="max-width:100%; background:#f0fdf4; border-color:#bbf7d0; display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
                        ...
                        <button class="btn btn-success" onclick="zavrsiCasKomplet()"><i class="fa-solid fa-square-check"></i> Заврши одржани час и сними дневник рада</button>
                    </div>
                        ...
                        <div class="admin-panes-box">
                            <div id="pane-predmeti" class="admin-pane active">
                                ...
                                <div id="pane-ucenici" class="admin-pane">
                                    ...
                                    <div class="form-group">
                                        <label>Име и презиме ђака:</label>
                                ...
                                <div id="pane-nastavnici" class="admin-pane">
                                    ...
                                    <div class="form-card" style="border:none; padding:0; max-width:100%;">
                                        ...
                                        <button class="btn btn-success" onclick="dodajNastavnikaSistem()"><i class="fa-solid fa-user-plus"></i> Сачувај и активирај профил наставника</button>
                                    </div>
                                    <br>
                                    <table class="es-table">
        ...
        <div class="modal-overlay" id="modal-ocena-upis">
            <div class="modal-box">
                <div class="modal-header">
                    ...
                    <span style="cursor:pointer;" onclick="zatvoriModal('modal-ocena-upis')">&times;</span>
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <label>Врста уноса:</label>
                    ...
                    <div class="form-group">
                        <label>Вредност / Оцена:</label>
                        <input type="text" class="form-control" id="nova-ocena-vrednost" value="5">
                    </div>
                    <div class="form-group">
                        <label>Категорија провере / Опис активности:</label>
                    ...
                    <button class="btn btn-success" style="width:100%; justify-content:center;" onclick="sacuvajOcenuĐakuUBazu()">Сачувај у дневник</button>
                </div>
            </div>
        </div>
        ...
        <script>
            ...
            let regPredmeti = [
                ...
                { id: "TEH", naziv: "Техника и технологија" },
                { id: "MAT", naziv: "Математика" }
            ];
            let odeljenjaLista = ["IV-3", "VIII-1", "V-2"];
            let sviUcenici = [];
            let sviCasovi = [];
            let listaNastavnika = [];
            ...
            function OsveziSelectMenijeIPanele() {
                ...
                ucitajUcenikeUMrezu();
                renderujCasoveUTimeline();
                IscrtajAdminTabele();
            }

            function ucitajUcenikeUMrezu() {
                let trenutnoOdeljenje = document.getElementById('global-odeljenje').value;
                document.getElementById('imenik-trenutno-odjenje').innerText = trenutnoOdeljenje;
                ...
                let filtrirani = sviUcenici.filter(u => u.odeljenje === trenutnoOdeljenje);
            ...
            async function obrisiUcenikaZauvek(event, id) {
                ...
                if(confirm("Да ли сте сигурни да желите да обришете овог ученика заувек из базе података?")) {
            ...
            async function zavrsiCasKomplet() {
                ...
                alert("Час је успешно затворен, евидентиран и сви изостанци су уписани!");
                aktivanCasPodaci = null;
                lokalniIzostanciNaCasu = {};
                document.getElementById('lekcija-naziv').value = "";
                document.getElementById('m-tok-casa').style.display = 'none';
                
                await osvežiSvePodatkeSaServera();
                switchPage('stranica-imenik');
            }
            ...
            function prikažiDosijeUčenika(id) {
                ...
                let prosek = brojOcenaZaProsek > 0 ? (zbir / brojOcenaZaProsek).toFixed(2) : "-";
                document.getElementById('dosije-ucenik-prosek').innerText = `Просек из предмета: ${prosek}`;
                ...
                izostanciKontejner.innerHTML = (u.izostanci || []).length === 0 ? "<li>Нема евидентираних изостанака.</li>" : "";
                    ...
                    izostanciKontejner.innerHTML += `
                        <li class="history-item" style="flex-direction:column; align-items:flex-start; gap:10px;">
                            <div style="display:flex; justify-content:space-between; width:100%;">
                                <span><strong>${iz.predmet}</strong> - ${iz.lekcija} (Датум: ${iz.datum})</span>
                                <span style="color:var(--es-${c}); font-weight:bold; text-transform:uppercase;">${iz.status}</span>
                            </div>
                ...
                if(listaVladanja.length === 0) {
                    vladanjeKontejner.innerHTML = `<p style="color:#718096; font-size:14px; padding:10px;">Нема напомена о владању.</p>`;
                } else {
                    listaVladanja.forEach((v, index) => {
                        let jeNapomena = (v.vrsta === 'Напомена');
                        let desniDeoHTML = jeNapomena ? `
                            <div class="vladanje-cas-kvadrat-es">
                                <span>${v.cas || '1'}.</span><small>Час</small>
                            </div>` : `<div style="padding:8px 12px; background:#fee2e2; border-radius:4px; font-weight:bold; color:var(--es-red); font-size:11px; text-transform:uppercase; border:1px solid #fca5a5;">Службена мера</div>`;

                        let naslovTrake = jeNapomena ? `${v.vrsta} - ${v.predmet || tp}` : `${v.vrsta}`;

                        vladanjeKontejner.innerHTML += `
                            <div class="vladanje-traka-es" style="${!jeNapomena && v.vrsta !== 'Похвала' ? 'border-left-color: var(--es-red); background:#fff5f5;' : ''}">
                                <div class="vladanje-info-es">
                                    <div class="vladanje-datum-es">Датум евиденције: <strong>${v.datum}</strong></div>
                                    <div class="vladanje-predmet-es" style="${jeNapomena ? 'color:var(--es-purple-text);' : 'color:var(--es-red);'}">${naslovTrake}</div>
                                    <div class="vladanje-tekst-es">${v.tekst}</div>
                                </div>
                                <div class="vladanje-desno-es">
                                    ${desniDeoHTML}
                                    <div class="vladanje-akcije-es">
                                        <button class="btn btn-secondary btn-xs" onclick="izmeniVladanjeUBazi('${u._id || u.id}', ${index})">Исправи</button>
                                        <button class="btn btn-danger btn-xs" onclick="obrisiVladanjeUBazi('${u._id || u.id}', ${index})">Обриши</button>
                                    </div>
                                </div>
                            </div>`;
                    });
            ...
            function osveziDosijeAkoJeOtvoren() {
                if(document.getElementById('stranica-dosije').classList.contains('active') && odabraniUcenikId) {
                    prikažiDosijeUčenika(odabraniUcenikId);
                } else { ucitajUcenikeUMrezu(); }
            }

            async function izmeniOcenuUBazi(uId, oId) {
                let u = sviUcenici.find(x => (x._id === uId || x.id == uId));
                let o = u.ocene.find(x => x.id === oId);
                let nV = prompt("Измените вредност:", o.vrednost);
                let nK = prompt("Измените опис / категорију:", o.vrsta);
                let nD = prompt("Измените датум (ГГГГ-ММ-ДД):", o.datum);
                if(nV !== null) o.vrednost = nV;
                if(nK !== null) o.vrsta = nK;
                if(nD !== null) o.datum = nD;
                await AzurirajUcenikaNaServeru(u);
            }

            async function obrisiOcenuUBazi(uId, oId) {
                if(confirm("Обрисати?")) {
                    let u = sviUcenici.find(x => (x._id === uId || x.id == uId));
                    u.ocene = u.ocene.filter(x => x.id !== oId); await AzurirajUcenikaNaServeru(u);
                }
            }

            async function izmeniIzostanakUBazi(uId, idx) {
                let u = sviUcenici.find(x => (x._id === uId || x.id == uId));
                let iz = u.izostanci[idx];
                let nL = prompt("Измените разлог:", iz.lekcija);
                let nD = prompt("Измените датум (ГГГГ-ММ-ДД):", iz.datum);
                if(nL !== null) iz.lekcija = nL;
                if(nD !== null) iz.datum = nD;
                await AzurirajUcenikaNaServeru(u);
            }

            async function obrisiIzostanakUBazi(uId, idx) {
                if(confirm("Обрисати изостанак?")) {
                    let u = sviUcenici.find(x => (x._id === uId || x.id == uId));
                    u.izostanci.splice(idx, 1); await AzurirajUcenikaNaServeru(u);
                }
            }

            async function promeniStatusIzostanka(uId, idx, noviStatus) {
                let u = sviUcenici.find(x => (x._id === uId || x.id == uId));
                u.izostanci[idx].status = noviStatus; await AzurirajUcenikaNaServeru(u);
            }

            async function dodajVladanjeUcenikuUBazu() {
                let u = sviUcenici.find(x => (x._id === odabraniUcenikId || x.id == odabraniUcenikId));
                let vrsta = document.getElementById('vladanje-vrsta-select').value;
                let tekst = document.getElementById('vladanje-tekst-input').value;
                let datum = document.getElementById('vladanje-datum-input').value || new Date().toISOString().split('T')[0];
                let cas = document.getElementById('vladanje-cas-input').value;
                let pr = document.getElementById('global-predmet').value;
                
                if(!tekst) return;
                if(!u.vladanje_lista) u.vladanje_lista = [];

                let noviZapis = { id: Date.now(), vrsta, tekst, datum };
                if(vrsta === 'Напомена') {
                    noviZapis.predmet = pr;
                    noviZapis.cas = cas;
                }

                u.vladanje_lista.push(noviZapis);
                document.getElementById('vladanje-tekst-input').value = "";
                await AzurirajUcenikaNaServeru(u);
            }

            async function izmeniVladanjeUBazi(uId, index) {
                let u = sviUcenici.find(x => (x._id === uId || x.id == uId));
                let v = u.vladanje_lista[index];
                let nT = prompt("Измените текст:", v.tekst);
                let nD = prompt("Измените датум (ГГГГ-ММ-ДД):", v.datum);
                if(nT !== null) v.tekst = nT;
                if(nD !== null) v.datum = nD;
                await AzurirajUcenikaNaServeru(u);
            }
            ...
            async function obrisiVladanjeUBazi(uId, index) {
                if(confirm("Обрисати?")) {
                    let u = sviUcenici.find(x => (x._id === uId || x.id == uId));
                    u.vladanje_lista.splice(index, 1); await AzurirajUcenikaNaServeru(u);
                }
            }

            async function sacuvajOcenuVladanja() {
                let u = sviUcenici.find(x => (x._id === odabraniUcenikId || x.id == odabraniUcenikId));
                u.ocena_vladanja = document.getElementById('zakljucno-vladanje-select').value;
                await AzurirajUcenikaNaServeru(u); alert("Успешно снимљено!");
            }

            async function obrisiCasUBazu(cId) {
                if(confirm("Обрисати час?")) { await fetch(`${API_URL}/api/casovi/${cId}`, { method: 'DELETE' }); await osvežiSvePodatkeSaServera(); }
            }

            async function izmeniCasUBazu(cId, staraLekcija) {
                let nL = prompt("Измените назив јединице:", staraLekcija);
                if(nL) {
                    await fetch(`${API_URL}/api/casovi/${cId}`, { method: 'PUT', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ lekcija: nL }) });
                    await osvežiSvePodatkeSaServera();
                }
            }

            function renderujCasoveUTimeline() {
                let lista = document.getElementById('lista-odrzanih-casova-timeline'); if(!lista) return; lista.innerHTML = "";
                if(sviCasovi.length === 0) { lista.innerHTML = `<div style="color:#718096; padding:15px; text-align:center;">Нема часова.</div>`; return; }
                sviCasovi.forEach(c => {
                    lista.innerHTML += `
            ...
            function IscrtajAdminTabele() {
                document.getElementById('tabela-admin-predmeti').innerHTML = regPredmeti.map((p, index) => `
                    <tr>
                        <td><strong>${p.id}</strong></td><td>${p.naziv}</td>
                        <td><button class="btn btn-danger btn-xs" onclick="obrisiPredmetSistem(${index})">Обриши</button></td>
                    </tr>`).join('');
                
                document.getElementById('tabela-admin-odeljenja').innerHTML = odeljenjaLista.map((o, index) => `
                    <tr>
                        <td><strong>${o}</strong></td><td>Активан разред</td>
                        <td><button class="btn btn-danger btn-xs" onclick="obrisiOdeljenjeSistem(${index})">Обриши</button></td>
                    </tr>`).join('');
                ...
                let tNast = document.getElementById('tabela-admin-nastavnici-lista');
                if(tNast) {
                    tNast.innerHTML = listaNastavnika.map(n => `
                        <tr>
                            <td><strong>${n.ime}</strong><br><small style="color:#718096;">${n.uloga}</small></td>
                            <td><code>${n.username}</code></td>
                            <td><button class="btn btn-danger btn-xs" onclick="obrisiNastavnikaSistem('${n._id || n.id}')">Обриши</button></td>
                        </tr>`).join('');
                }
            }

            function obrisiPredmetSistem(index) {
                if(confirm("Обрисати предмет заувек?")) { regPredmeti.splice(index, 1); OsveziSelectMenijeIPanele(); }
            }

            function obrisiOdeljenjeSistem(index) {
                if(confirm("Обрисати одељење заувек?")) { odeljenjaLista.splice(index, 1); OsveziSelectMenijeIPanele(); }
            }

            async function dodajNastavnikaSistem() {
                let ime = document.getElementById('admin-nastavnik-ime').value;
                let uloga = document.getElementById('admin-nastavnik-uloga').value;
                let username = document.getElementById('admin-nastavnik-user').value;
                let password = document.getElementById('admin-nastavnik-pass').value;
                ...
                await fetch(`${API_URL}/api/nastavnici`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ ime, uloga, username, password, odeljenja, predmeti })
                });
                ...
                await osvežiSvePodatkeSaServera(); alert("Наставник уписан у базу!");
            }

            async function obrisiNastavnikaSistem(id) {
                if(confirm("Обрисати налог наставника?")) { await fetch(`${API_URL}/api/nastavnici/${id}`, { method: 'DELETE' }); await osvežiSvePodatkeSaServera(); }
            }

            function dodajNoviPredmetSistem() {
                ...
                regPredmeti.push({ id: sifra, naziv: n }); document.getElementById('admin-novi-predmet-input').value = ""; OsveziSelectMenijeIPanele();
            }
            
            function dodajNovoOdeljenjeSistem() {
                let o = document.getElementById('admin-novo-odeljenje-input').value; if(!o) return;
                if(!odeljenjaLista.includes(o)) odeljenjaLista.push(o); document.getElementById('admin-novo-odeljenje-input').value = ""; OsveziSelectMenijeIPanele();
            }
            
            async function dodajUcenikaIzAdminPanela() {
                let ime = document.getElementById('admin-ucenik-ime-input').value;
                let odeljenje = document.getElementById('global-odeljenje').value;
                if(!ime) return;
                await fetch(`${API_URL}/api/ucenici`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ ime, odeljenje, ocene:[], izostanci:[], aktivnosti:[], vladanje_lista:[], ocena_vladanja:"5" })
                });
            ...
            async function sacuvajOcenuĐakuUBazu() {
                let u = sviUcenici.find(x => (x._id === odabraniUcenikId || x.id == odabraniUcenikId));
                let tip = document.getElementById('nova-ocena-vrsta-input').value;
                let val = document.getElementById('nova-ocena-vrednost').value;
                let kat = document.getElementById('nova-ocena-kategorija').value;
                let pr = document.getElementById('global-predmet').value;
                let datum = document.getElementById('nova-ocena-datum-input').value || new Date().toISOString().split('T')[0];
                
                if(!u.ocene) u.ocene = [];
                u.ocene.push({ id: Date.now(), tip, vrednost: val, vrsta: kat, predmet: pr, datum });
                zatvartimodal = zatvoriModal('modal-ocena-upis'); 
                await AzurirajUcenikaNaServeru(u);
            }

            async function AzurirajUcenikaNaServeru(u) {
                await fetch(`${API_URL}/api/ucenici/${u._id || u.id}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(u)
                });
                await osvežiSvePodatkeSaServera();
                osveziDosijeAkoJeOtvoren();
            }

            function izvrsiPrijavuNaSistem() {
                let u = document.getElementById('sys-auth-user').value;
                let p = document.getElementById('sys-auth-pass').value;
                
                let nadjen = listaNastavnika.find(x => x.username === u && x.password === p);
                
                if(nadjen || (u === 'admin' && p === 'admin123')) {
                    document.getElementById('stranica-login-sistema').style.display = 'none';
                    document.getElementById('glavni-interfejs-aplikacije').style.display = 'block';
                    OsveziSelectMenijeIPanele();
                } else {
                    alert("Нетачно корисничко име или лозинка!");
                }
            }
            ...
            function switchPage(pageId) {
                ...
                let t = document.getElementById(pageId); if(t) t.classList.add('active');
                if(pageId === 'stranica-imenik') document.getElementById('m-imenik').classList.add('active');
                if(pageId === 'stranica-upis') document.getElementById('m-upis').classList.add('active');
                if(pageId === 'stranica-tok-casa') document.getElementById('m-tok-casa').classList.add('active');
                if(pageId === 'stranica-izvestaji') document.getElementById('m-izvestaji').classList.add('active');
            }
            ...
            function otvoriProzorZaOcenjivanje() { document.getElementById('modal-ocena-upis').style.display = 'flex'; }
            function zatvoriModal(id) { document.getElementById(id).style.display = 'none'; }
            function snimiSveZakljucne() { let v = document.getElementById('zakljucna-ocena-vrednost').value; alert(v ? `Успешно снимљена закључна оцена: ${v}` : "Изаберите оцену!"); }
        </script>
</body>
</html>
